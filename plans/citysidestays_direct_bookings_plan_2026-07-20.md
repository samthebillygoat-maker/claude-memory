...
**HARDENED IMPLEMENTATION PLAN (v2):** `C:\Users\samth\cityside-stays\docs\superpowers\plans\2026-07-20-cityside-direct-bookings.md` — pressure-tested by a 6-lens senior-review workflow (49→36 verified findings) + FLUX FUSION panel (PASS/9). Phase 1 is a REAL payments+inventory system: needs Vercel Postgres (quotes/holds/bookings/stripe_events) + Upstash (rate-limit). Race handled by a Hostaway calendar HOLD at checkout + Stripe manual-capture (cancel = zero fee on conflict) + refund fallback. Two LAUNCH-BLOCKER spikes: SPIKE-H (Hostaway channelId/hold/auto-block semantics) + SPIKE-TAX (FL county sales+surtax+bed-tax rates). Key gotchas caught: Next 14.2 fetch-cache serves stale availability (use `cache:'no-store'`), unpaid-session bookings (gate on `payment_status==='paid'`), turnover-night off-by-one, `git add` before `.gitignore` leaks secrets, durable idempotency must be claim→commit not SETNX-before-work.
